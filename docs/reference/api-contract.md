...
:warning: **[GAP]** The router category has an extra `setup_url` key; the others don't. Deserialize defensively.
...
(The page's `description` erroneously reads "Returns status of the Default Rule." — copy-paste from the GET.)
- Path: `profile_id` (string, required)
...
- Response key: **`body.filters`** — example is an array of filter-name strings: `["malware","ads"]`
...
| Field | Type | Required | Notes |
|---|---|---|---|
| `PK` | **integer** | yes | **folder ID** — note: integer, unlike profile/rule/device PKs which are strings |
...
| `hostnames[]` | array | **yes** | example here is the bare string `"domain1.com"`, not an array — docs sloppiness; it is declared `type: array` |
...
- Response key: **`body.services`** (array): `PK`, `name`, `category`, `unlock_location` (all required);
  `locations` (array of string) and `warning` (string) optional.
...
- Response schema is **empty `{}`**. **[GAP]** — presumably `body.access` or `body.ips`. **Probe this; the
  response key is genuinely unknown.**
...
- :warning: **[CONFLICT] Seats.** The page's prose is explicitly about seats:
  ...
  "including seats". Either the schema is incomplete (most likely) or seats can't be set here. **Probe — and
  be careful, because getting this wrong triggers real billing.**
