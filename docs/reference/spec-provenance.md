...
- **All 46 operations lack an `operationId`** — blocks clean codegen; we assign our own.
...
The spec settles this: **18 writes are `application/x-www-form-urlencoded`; exactly one
...
- `body` has **three** shapes, not one (`/users` and `/ip` are flat; `/network` has siblings).
