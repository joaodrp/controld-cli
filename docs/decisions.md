...
`--json` is explicit. Piping changes *rendering* (color, pager), never *representation* — what `gh` and
...
whose target-specific upstream data sits with each target; members individually nullable (the
...
refused; an existing group/world-readable file draws a warning. Windows relies on `%APPDATA%`'s
...
forms require a non-GET `-X <method>`** (a GET never carries a body here); the two are mutually
...
No `lib.rs`. Our public API is the CLI contract (exit codes, JSON schema — D2, D5), not Rust
types; a lib target on crates.io would create semver obligations nobody asked for (fd does the
same; bat's lib is deliberate, ours would be accidental). If a reusable client ever emerges, it
